...
const emptyForm: PdfForm = { name: '', description: '', cover_image_url: '', drive_link: '', pdf_category_id: 'none', cover_image_file: null };
  ...
  <div className="space-y-4">
    ...
    <div>
      ...
      <ImageUpload
        ...
        onChange={(url, file) => setFormData({ ...formData, cover_image_url: url, cover_image_file: file || null })}
      />
    </div>
  </div>
);
...
export default function PdfsManager({ search }: { search: string }) {
    ...
    if (targetEditId && pdfs) {
      ...
      if (pdfToEdit) {
        ...
        setEditForm({
          ...
          pdf_category_id: pdfToEdit.pdf_category_id || 'none'
        });
      }
    }
  }, [searchParams, pdfs]);
  ...
  const addPdf = useMutation({
    mutationFn: async () => {
      ...
      if (form.cover_image_file) {
        const uploadedUrl = await uploadFile('pdf-covers', form.cover_image_file);
  ...
  const remove = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['pdfs'] });
      toast.success('PDF deleted completely');
    },
    onError: (e: Error) => toast.error(e.message),
  });
        ...
        {filtered?.map((pdf: any) => (
          <div key={pdf.id} className="glass-card p-3 rounded-xl flex items-center justify-between gap-4 border border-border/40 hover:border-blue-500/30 transition-all group">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {pdf.author_avatar_url && (
                    <img src={pdf.author_avatar_url} alt="" className="w-6 h-6 rounded-full ml-1 border border-border/50 object-cover shadow-sm" />
                  )}
            ...
            <div className="flex gap-2 shrink-0 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
              <button
                onClick={() => {
                  ...
                  setEditForm({
                    ...
                    pdf_category_id: pdf.pdf_category_id || 'none'
                  });
                }}
              ...
              <button
                onClick={() => { if (window.confirm(`Delete "${pdf.name}"?`)) remove.mutate(pdf); }}
