...
const emptyForm: UpdateForm = { headline: '', content: '', image_url: '', external_link: '', image_file: null };
  ...
  <div className="space-y-4">
    <div>
      ...
      <Input
        ...
        placeholder="e.g. Exam Dates Announced!"
      />
    </div>
    <div>
    ...
    <div>
      ...
      <ImageUpload
        ...
        onChange={(url, file) => setFormData({ ...formData, image_url: url, image_file: file || null })}
      />
    </div>
  </div>
);
...
export default function UpdatesManager({ search }: { search: string }) {
  ...
  const addUpdate = useMutation({
    mutationFn: async () => {
      ...
      let authorName = 'EduDock Official';
      let authorAvatar = null;

      if (session?.user?.user_metadata) {
        authorName = session.user.user_metadata.full_name || authorName;

        let rawUrl = session.user.user_metadata.avatar_url;
        if (rawUrl && rawUrl.includes('googleusercontent.com') && rawUrl.includes('=s')) {
          rawUrl = rawUrl.replace(/=s\d+-c/g, '=s500-c');
        }
        authorAvatar = rawUrl || null;
      }
  ...
  const updateExisting = useMutation({
    mutationFn: async () => {
      // Get the current update to check for old image
      ...
      if (editForm.image_file) {
        ...
        if (currentUpdate?.image_url && currentUpdate.image_url.includes('supabase.co')) {
          try {
            const url = new URL(currentUpdate.image_url);
            const pathParts = url.pathname.split('/');
            const bucketIndex = pathParts.indexOf('update-images');
            if (bucketIndex !== -1) {
              const filePath = pathParts.slice(bucketIndex + 1).join('/');
              await supabase.storage.from('update-images').remove([filePath]);
            }
          } catch (err) {
      ...
      const payload = {
        ...
        slug: generateSlug(editForm.headline),
  ...
  const remove = useMutation({
    mutationFn: async (update: any) => {
      // 1. DELETE FROM STORAGE FIRST (Double-Delete)
      if (update.image_url && update.image_url.includes('supabase.co')) {
        try {
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['updates'] });
      toast.success('Update deleted completely (storage + database)');
    },
    onError: (e: Error) => toast.error(e.message),
  });
          ...
          <div key={update.id} className="glass-card p-3 rounded-xl flex items-center justify-between gap-4 border border-border/40 hover:border-purple-500/30 transition-all group">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  {update.author_avatar_url && (
                    <img src={update.author_avatar_url} alt="" className="w-6 h-6 rounded-full border border-border/50 object-cover shadow-sm" />
                  )}
            ...
            <div className="flex gap-2 shrink-0 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
              <button
                ...
                className="p-2.5 rounded-xl bg-purple-500/10 text-purple-500 hover:bg-purple-500/20 transition-colors"
                title="Edit Update"
              >
                <Pencil className="h-4 w-4" />
              </button>
              <button
                onClick={() => { if (window.confirm(`Delete "${update.headline}"?`)) remove.mutate(update); }}
