  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        ...
        // 🚨 HIGH-RES GOOGLE PROFILE PICTURE INTERCEPTOR 🚨
        let avatar = meta?.avatar_url;
        if (avatar && avatar.includes('googleusercontent.com') && avatar.includes('=s')) {
          // Replaces the tiny =s96-c with massive =s500-c
          avatar = avatar.replace(/=s\d+-c/g, '=s500-c');
        } else if (!avatar) {
          avatar = `https://api.dicebear.com/7.x/initials/svg?seed=${name}&backgroundColor=8b5cf6&textColor=ffffff`;
        }
        ...
        <div className="p-4 border-t border-border/50 flex flex-col gap-2">
          ...
          {userProfile && (
            <div className={`flex items-center gap-3 p-2 mb-2 ${isCollapsed ? 'justify-center' : ''}`}>
              <img src={userProfile.avatar} alt="Admin" className="w-11 h-11 rounded-full border border-border/50 shrink-0 object-cover bg-muted" />
              {!isCollapsed && (
